"""Scout OpenEnv seeds and bucket them by reward behavior.

This utility probes deterministic OpenEnv seeds with the current policy model,
records reward statistics per seed, and writes a manifest that grpo_train.py
can use to train on a selected bucket such as `mixed`.
"""
...
def _load_model_for_scout(model_name: str, torch_dtype):
    """Load model with a local-cache-first strategy."""
    attempts = [
